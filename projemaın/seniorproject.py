    ...
    while(True):
        ...
        if ((starttime+10)<time.time()):
            if(situation==False):
                ...
                if (counter == 8):
                    im = cv2.imread('seftalı.jpg')
                if (counter == 9):
                    im = cv2.imread('kayısı.jpg')
                if (counter == 10):
                    im = cv2.imread('kiraz.jpg') 
                if (counter >=10):
                    im = cv2.imread('outofmylimit.jpg')
...
def eyeControl(gray, faces,frame):
   ...
   #    find the lenght for both horizontal and vertical line
        ...
        L_ratio = L_hori_lenght/(L_ver_lenght+0.0000000000001)
        R_ratio = R_hori_lenght/(R_ver_lenght+0.0000000000001)
